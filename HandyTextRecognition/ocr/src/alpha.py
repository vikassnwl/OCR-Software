...
def rgb(img):
    ...
    cv2.imwrite("ocr/puts/bl_text.jpg", iblack)
...
def dilate_erode():
    ...
    for i in files:
        ...
        # cv2.imwrite('Input', img)
        cv2.imwrite('ocr/ze/erode/Erode_{}.jpg'.format(i[:len(i) - 4]), img_erosion)
        cv2.imwrite('ocr/ze/erode1/Erode_{}.jpg'.format(i[:len(i) - 4]), img_er)
        cv2.imwrite('ocr/ze/dilate/Dilate_{}.jpg'.format(i[:len(i)-4]), img_dilation)
        cv2.imwrite('ocr/ze/gauss/gauss_{}.jpg'.format(i[:len(i)-4]), gausBlur)
        cv2.imwrite('ocr/ze/median/median_{}.jpg'.format(i[:len(i)-4]), medBlur)
        cv2.imwrite('ocr/ze/bifilter/bi_{}.jpg'.format(i[:len(i)-4]), bilFilter)
        cv2.imwrite('ocr/ze/di_/di_{}.jpg'.format(i[:len(i) - 4]), di2)
        cv2.imwrite('ocr/ze/er_/er_{}.jpg'.format(i[:len(i)-4]), er2)
        cv2.imwrite('ocr/ze/er_bi_/bi_{}.jpg'.format(i[:len(i)-4]), bi_)
        cv2.imwrite('ocr/ze/bid/bid_{}.jpg'.format(i[:len(i)-4]), bi_di_)
        cv2.imwrite('ocr/ze/s1/s1_{}.jpg'.format(i[:len(i)-4]), sharp1)
        cv2.imwrite('ocr/ze/s2/s2_{}.jpg'.format(i[:len(i)-4]), sharp2)
        cv2.imwrite('ocr/ze/gauss_sh/gsh_{}.jpg'.format(i[:len(i)-4]), sharp3)
        cv2.imwrite('ocr/ze/er_bi_sh/bsh_{}.jpg'.format(i[:len(i)-4]), sharp4)
        cv2.imwrite('ocr/ze/median_sh/msh_{}.jpg'.format(i[:len(i)-4]), sharp5)
        cv2.imwrite('ocr/ze/bid_sh/bidsh_{}.jpg'.format(i[:len(i)-4]), sharp6)
        cv2.imwrite('ocr/ze/morph/morph_{}.jpg'.format(i[:len(i)-4]), result)
    ...
    for d in text_data:
        ...
        for e, p, q, r, s in zip(df['ImageName'], df['x-60'], df['y-20'], df['x+w+60'], df['y+h']):
            ...
            if d[0] == pre+e:
                ...
                cv2.imwrite('ocr/puts/put_text_{}.jpg'.format(col),text)
                count += 1
...
def run():
    ...
    for j in img_files:
        if j.endswith("ib.jpg"):
            color = "blue"
            text_init("ocr/outputs/"+j,"ocr/resize/morph",color)
        # if j.endswith("ir.jpg"):
        #     print("{} image is in process".format(j))
        #     color = "red"
        #     text_init("../outputs/"+j,"ocr/resize/morph",color)
        # if j.endswith("ig.jpg"):
        #     print("{} image is in process".format(j))
        #     color = "green"
        #     text_init("../outputs/"+j,"ocr/resize/bid_sh",color)
        # if j.endswith("bl_text.jpg"):
        #     color = "black"
        #     text_init("../outputs/"+j,"ocr/resize/gauss_sh",color)
        else:
